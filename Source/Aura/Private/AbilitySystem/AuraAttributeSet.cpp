...
{
	//Создаётся alias, псевдоним - ссылка на синглтон FAuraGameplayTags
...
void UAuraAttributeSet::GetLifetimeReplicatedProps(TArray<FLifetimeProperty>& OutLifetimeProps) const
{
	...
	/** * Настройка сетевой рассылки (DOREPLIFETIME). Эта функция срабатывает один раз при создании объекта (на сервере и на клиенте).
	* Её задача: Просто сказать движку: «Слушай, переменная Strength — особенная. Каждый раз, когда она меняется на сервере, её нужно упаковать в пакет и отправить клиентам».
	...
	if (Data.EvaluatedData.Attribute == GetIncomingDamageAttribute())
	{
		...
		if (LocalIncomingDamage > 0.f)
		{
			...
			else
			{
				...
				TagContainter.AddTag(FAuraGameplayTags::Get().Effects_HitReact);
				Props.TargetASC->TryActivateAbilitiesByTag(TagContainter);
			}
